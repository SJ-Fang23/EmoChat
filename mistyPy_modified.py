...
class Robot:
    ...
    def changeLED(self,red,green,blue):
        ...
        feedback = requests.post('http://'+self.ip+'/api/led',json={"red": red,"green": green,"blue": blue})
        # print(feedback)
    ...
    def moveArms(self, arm, angle):
        assert arm in ["left","right"], "arm is neither left nor right!"
        assert angle in range(-360,360), "angle should be in range of -360 to 360!"
        requests.post('http://' + self.ip + '/api/arms', json = {"Arm":arm, "Position":angle, "Velocity":20})
